...
public class GameCodeGenerator {
  ...
  private String createGameCode() {
    ...
    String gameCode = "";
    for (int i = 0; i < 6; i++) gameCode += (char) (random.nextInt(65, 91));
    return gameCode;
  }

  /**
   * Given a set of existingGameCodes, continuously generates new game codes until one not present
   * in the set is created (in order to create a new, non-overlapping game)
   *
   * @param existingGameCodes : the set of existingGameCodes already within use in other games
   * @return the new (not previously in existence) game code to be used within a game being
